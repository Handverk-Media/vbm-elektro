...
const KATEGORIER = ["Alle", "Elbillading", "Elkontroll", "Smarthus", "Regelverk"]
...
export default function BloggPage() {
  return (
    <div className="min-h-screen bg-[#F7F6F3]">
      ...
      {/* Hero */}
      <section className="pt-20 bg-[#1A1A1A]">
      ...
      <section className="py-16">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {INNLEGG.map((innlegg, i) => (
              <Link
                key={innlegg.slug}
                href={`/blogg/${innlegg.slug}`}
              ...
              >
                {/* Farge-topp basert på kategori */}
                <div className={`h-2 ${i % 3 === 0 ? "bg-[#E1342B]" : i % 3 === 1 ? "bg-[#1A1A1A]" : "bg-[#EEEDE8]"}`} />
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-xs font-semibold text-[#E1342B] tracking-wide uppercase">
                      {innlegg.kategori}
                    </span>
                  ...
                  <p className="text-sm text-[#6B6B6B] leading-relaxed mb-5 line-clamp-3">
                    {innlegg.ingress}
                  </p>
      ...
      <section className="bg-[#EEEDE8] py-16">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <p className="text-[#6B6B6B] mb-8 max-w-md mx-auto">
            Vi dekker Bærum, Oslo og Asker. Gratis befaring og fast pris.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/book"
              className="bg-[#E1342B] text-white font-bold px-8 py-3.5 rounded-lg hover:bg-[#c42d24] transition-colors inline-flex items-center justify-center gap-2"
            >
              Book gratis befaring
            </Link>
            <a
              href={`tel:${BEDRIFT.telefon.replace(/\s/g, "")}`}
              className="border border-[#1A1A1A]/20 text-[#1A1A1A] font-semibold px-8 py-3.5 rounded-lg hover:border-[#1A1A1A]/40 transition-colors inline-flex items-center justify-center gap-2"
            >
              {BEDRIFT.telefon}
            </a>
          </div>
